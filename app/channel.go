...
func (a *App) AddChannelMemberByUser(channelId string,user *model.User,isAdmin bool)(*model.AppError){
	channelMember,_ := a.GetChannelMember(channelId,user.Id)
	...
	if err := a.Srv.Store.ChannelMemberHistory().LogJoinEvent(user.Id, channelId, model.GetMillis()); err != nil {
...
func (a *App) DisbandChannelMembers(channel *model.Channel, userId string) *model.AppError {
	channelMembers,err := a.Srv.Store.Channel().GetMember(channel.Id, userId)
	if err != nil{
		return err
	}
	if channelMembers.SchemeAdmin != true {
		err := model.NewAppError("disbanbChannel", "api.channel.disbanb_Insufficient permissions_error", nil, "权限不足", http.StatusBadRequest)
		return err
	}
	...
	if err := a.Srv.Store.Channel().DisbandChannelMembers(channel.Id); err != nil{
		return err
	}
	a.InvalidateCacheForChannel(channel)
...
func (a *App) GetChannelOnlyByName(channelName string) (*model.Channel, *model.AppError) {
	var channel *model.Channel
	var err *model.AppError

	channel, err = a.Srv.Store.Channel().GetOnlyByChannelName(channelName)
...
func (a *App) GetAllChannelsForPosts(page, perPage int) (*model.ChannelListWithPostData, *model.AppError) {
	return a.Srv.Store.Channel().GetAllChannelsForPosts(page*perPage, perPage)
}
...
func (a *App) GetChannelMemberMentions(channelId string, userId string) (*model.ChannelMember, *model.AppError) {
	return a.Srv.Store.Channel().GetChannelMemberMentions(channelId,userId)
}
...
func (a *App) PostToPersonMessage(userId ,batchId string, request *model.IncomingWebhookRequest,channel *model.Channel) *model.AppError {
	user, err := a.Srv.Store.User().Get(userId)
	if err != nil {
		return model.NewAppError("PostUpdateChannelPurposeMessage", "app.channel.post_update_channel_purpose_message.retrieve_user.error", nil, err.Error(), http.StatusBadRequest)
	}
	...
	text := linkWithTextRegex.ReplaceAllString(request.Text, "[${2}](${1})")
	post := &model.Post{
		ChannelId: channel.Id,
		Message:   text,
		Type:      "custom_Msg",
	...
	if len(request.Username) != 0 {
		post.AddProp("override_username", request.Username)
	} else {
		post.AddProp("override_username", model.DEFAULT_WEBHOOK_USERNAME)
	}
	if len(request.IconURL) != 0 {
		post.AddProp("override_icon_url", request.IconURL)
	}
	post.AddProp("from_bot", "true")
	request.Attachments = a.ProcessSlackAttachments(request.Attachments)
	// attachments is in here for slack compatibility
	if len(request.Attachments) > 0 {
		request.Props["attachments"] = request.Attachments
	}

	if _, err := a.CreatePost(post, channel, false); err != nil {
		return model.NewAppError("", "app.channel.post_update_channel_purpose_message.post.error", nil, err.Error(), http.StatusInternalServerError)
	}
	 a.PreparePostForClient(post, true, false)
	return nil
}
