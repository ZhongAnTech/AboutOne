...
func testChannelStoreIncrementMentionCount(t *testing.T, ss store.Store) {
	...
	err = ss.Channel().IncrementMentionCount(m1.UserId,m1.ChannelId, m1.UserId)
	if err != nil {
		t.Fatal("failed to update")
	}

	err = ss.Channel().IncrementMentionCount(m1.UserId,m1.ChannelId, "missing id")
	if err != nil {
		t.Fatal("failed to update")
	}

	err = ss.Channel().IncrementMentionCount(m1.UserId,"missing id", m1.UserId)
	if err != nil {
		t.Fatal("failed to update")
	}

	err = ss.Channel().IncrementMentionCount(m1.UserId,"missing id", "missing id")
	if err != nil {
		t.Fatal("failed to update")
	}
}
