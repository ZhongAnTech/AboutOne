...
func ParamsFromRequest(r *http.Request) *Params {
	...
	if val, ok := props["collect_id"]; ok {
		params.CollectId = val
	}
	...
	if val, ok := props["meeting_id"]; ok {
		params.MeetingId = val
	} else {
		params.MeetingId = query.Get("meeting_id")
	}
	if val, ok := props["file_type"]; ok {
		params.FileType = val
	} else {
		params.FileType = query.Get("file_type")
	}

	if val, ok := props["version"]; ok {
		params.Version = val
	} else {
	...
	if val, ok := props["file_id"]; ok {
		...
		params.FileId = query.Get("file_id")
	}
