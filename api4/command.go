...
func (api *API) InitCommand() {
	...
	api.BaseRoutes.Commands.Handle("/types", api.ApiSessionRequired(getCommandType)).Methods("GET")
...
func listCommands(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if customOnly {
		...
		commands, err = c.App.ListTeamCommandsByType(teamId,commandType)
		if err != nil {
			c.Err = err
			return
		}
...
func getCommandType(c *Context, w http.ResponseWriter, r *http.Request) {
	teamId := r.URL.Query().Get("team_id")
	...
	types, err := c.App.GetCommandTypes(teamId)
    if err!=nil{
    	mlog.Error("get command types failed! err="+err.Error())
		w.Write([]byte("[]"))
    	return
	}
	result,_ :=json.Marshal(types)
