...
func (a *App) GetUserEntityByUsername(username string) (*model.User, *model.AppError) {
	user,err := a.Srv.Store.User().GetEntityByUsername(username)
...
func (a *App) GetUpdateUsers(options *model.UserGetOptions) ([]*model.User, *model.AppError) {
	result := <-a.Srv.Store.User().GetAllUpdateProfiles(options)
	if result.Err != nil {
		return nil, result.Err
	}
	return result.Data.([]*model.User), nil
...
func (a *App) GetUsersPage(options *model.UserGetOptions, asAdmin bool) ([]*model.User, *model.AppError) {
	...
	for _,user :=range users{
		a.SSO.GetUserDepartmentInfo(user)
	}
...
func (a *App) GetDefaultProfileImage(user *model.User) ([]byte, *model.AppError) {
	...
	if user.IsBot {
		...
		img = model.MaleDefaultImage
		if user.Gender == "F" {
			img = model.FemaleDefaultImage
		}
	}
	if appErr != nil {
		return nil, appErr
