...
GO=go
DELVE=dlv
LDFLAGS += -X "za-white-screen/model.BuildNumber=$(BUILD_NUMBER)"
LDFLAGS += -X "za-white-screen/model.BuildDate=$(BUILD_DATE)"
LDFLAGS += -X "za-white-screen/model.BuildHash=$(BUILD_HASH)"
LDFLAGS += -X "za-white-screen/model.BuildHashEnterprise=$(BUILD_HASH_ENTERPRISE)"
LDFLAGS += -X "za-white-screen/model.BuildEnterpriseReady=$(BUILD_ENTERPRISE_READY)"

# GOOS/GOARCH of the build host, used to determine whether we're cross-compiling or not
BUILDER_GOOS_GOARCH="$(shell $(GO) env GOOS)_$(shell $(GO) env GOARCH)"

PLATFORM_FILES="./cmd/aboutone/main.go"
...
debug-server: start-docker
	...
	$(DELVE) debug $(PLATFORM_FILES) --build-flags="-ldflags '\
		-X za-white-screen/model.BuildNumber=$(BUILD_NUMBER)\
		-X \"za-white-screen/model.BuildDate=$(BUILD_DATE)\"\
		-X za-white-screen/model.BuildHash=$(BUILD_HASH)\
		-X za-white-screen/model.BuildHashEnterprise=$(BUILD_HASH_ENTERPRISE)\
		-X za-white-screen/model.BuildEnterpriseReady=$(BUILD_ENTERPRISE_READY)'"
