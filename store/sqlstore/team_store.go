...
func (s SqlTeamStore) GetTeamsByName(name string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		var data []*model.Team
		if _, err := s.GetReplica().Select(&data, "SELECT Teams.* FROM Teams WHERE  Teams.Name = :teamName AND Teams.DeleteAt = 0", map[string]interface{}{"teamName": name}); err != nil {
			result.Err = model.NewAppError("SqlTeamStore.GetTeamsByName", "store.sql_team.get_all.app_error", nil, err.Error(), http.StatusInternalServerError)
			return
		}
