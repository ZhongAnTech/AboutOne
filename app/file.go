...
import (
	...
	"za-white-screen/services/filesstore"
...
func (a *App)CopyFile(oldPath,newPath string) *model.AppError  {
	backend, err := a.FileBackend()
	if err != nil {
		return err
	}
	return backend.CopyFile(oldPath, newPath)
}
...
func (t *uploadFileTask) init(a *App) {
	...
	t.fileinfo.ChannelId = t.ChannelId //cml-20190729 保存ChannelId
...
func (a *App) UploadFileX(channelId, name string, input io.Reader,
	...
	//save file
	if *a.Config().FileSettings.EnableCdms {
		cdmsFileHash,uploadErr := a.Cdms.CdmsUpload(t.newReader(),t.fileinfo.Name,t.fileinfo.Extension,t.fileinfo.ChannelId)
		if uploadErr != nil {
			return nil, uploadErr
		}
		if cdmsFileHash != ""{
			t.fileinfo.Hash=cdmsFileHash
		}else {
			return nil,t.newAppError("api.file.upload_file.cdms_upload_fail.app_error", "文件上传至cdms后未返回hashid", http.StatusNotImplemented)
		}
	}else {
	...
	//file classification
	if strings.Contains(*a.Config().FileSettings.FileTypeDocument,"."+t.fileinfo.Extension){
		t.fileinfo.Classification = model.CLASSIFICATION_DOCUMENT
	}else if strings.Contains(*a.Config().FileSettings.FileTypeImage,"."+t.fileinfo.Extension){
		t.fileinfo.Classification = model.CLASSIFICATION_IMAGE
	}else if strings.Contains(*a.Config().FileSettings.FileTypeVideo,"."+t.fileinfo.Extension){
		t.fileinfo.Classification = model.CLASSIFICATION_VIDEO
	}else if strings.Contains(t.fileinfo.MimeType,model.CLASSIFICATION_IMAGE+"/"){
		t.fileinfo.Classification = model.CLASSIFICATION_IMAGE
	}else if strings.Contains(t.fileinfo.MimeType,model.CLASSIFICATION_VIDEO+"/"){
		t.fileinfo.Classification = model.CLASSIFICATION_VIDEO
	}else{
		t.fileinfo.Classification = model.CLASSIFICATION_OTHER
	}
...
//cml-20190904 分channel统计
func (a *App) SearchFileCountByChannels(channelIds []string,term string) (model.SearchChatCountRows, *model.AppError){
	return a.Srv.Store.FileInfo().SearchFileCountByChannels(channelIds,term)
}

//cml-20190904 根据channel查询文件+模糊搜索
func(a *App) SearchFileByChannels(channelIds []string,term string,page int, perPage int) ([]*model.FileInfoForDisk, *model.AppError){
	return a.Srv.Store.FileInfo().SearchFileByChannels(channelIds,term,page*perPage, perPage)
}

//cml-20190729 根据channelId获取文件列表-分页-可查询到无postid
func (a *App) GetForChannelIdAll(channelId string, page int, perPage int) ([]*model.FileInfoForDisk, *model.AppError) {
	return a.Srv.Store.FileInfo().GetForChannelIdAll(channelId, page*perPage, perPage)
}

//cml-20190729 根据channelId获取文件列表-分页
func (a *App) GetForChannelId(channelId string, page int, perPage int) ([]*model.FileInfoForDisk, *model.AppError) {
	return a.Srv.Store.FileInfo().GetForChannelId(channelId, page*perPage, perPage)
}

//cml-20190729 根据channelId获取文件列表-总记录数
func (a *App) GetForChannelIdCount(channelId string) (int64, *model.AppError) {
	return a.Srv.Store.FileInfo().GetForChannelIdCount(channelId)
}

//cml-20190902 根据channelIds和文件名(模糊)获取文件列表-分页
func (a *App) GetForChannelIdsForName(channelIds []string,name string, page int, perPage int) ([]*model.FileInfoForDisk, *model.AppError) {
	return a.Srv.Store.FileInfo().GetForChannelIdsForName(channelIds,name, page*perPage, perPage)
}

//cml-20190902 根据channelIds和文件名(模糊)获取文件列表-总记录数
func (a *App) GetForChannelIdForNameCount(channelIds []string,name string) (int64, *model.AppError) {
	return a.Srv.Store.FileInfo().GetForChannelIdForNameCount(channelIds,name)
}

//cml-20190729 根据FileId物理删除
func (a *App) DeleteByFileId(fileId string) (string, *model.AppError) {
	return a.Srv.Store.FileInfo().DeleteByFileId(fileId)
}

//cml-20191016 分类查询
func (a *App) ClassificationQuery(fileClassificationReq *model.FileClassificationReq) ([]*model.FileInfo, *model.AppError){
	return a.Srv.Store.FileInfo().ClassificationQuery(fileClassificationReq)
}

//cml-20191017 分页查询分类为空fileInfo
func (a *App) GetAllByClassificationIsEmpty(offset int, limit int) ([]*model.FileInfo, *model.AppError){
	return a.Srv.Store.FileInfo().GetAllByClassificationIsEmpty(offset,limit)
}
//cml-20191017 更新文件分类
func (a *App) UpdateFileClassification(fileId string, classification string) *model.AppError{
	return  a.Srv.Store.FileInfo().UpdateFileClassification(fileId,classification)
}
...
func (a *App) CopyFileInfosToForwardPost(userId ,channelId string, fileIds []string) ([]string, *model.AppError) {
	var newFileIds []string
	...
	for _, fileId := range fileIds {
		...
		fileInfo.UpdateAt = now
		fileInfo.PostId = ""
		//lgx-20101011 跨频道文件赋权
		if *a.Config().FileSettings.EnableCdms && fileInfo.Hash != "" && fileInfo.ChannelId != channelId {
			bool,authorizeAddErr := a.Cdms.AuthorizeAdd(fileInfo.Hash,channelId,einterfaces.CDMS_AUTHORITY_SOURCE_DOWNLOAD)
			if authorizeAddErr != nil {
				return nil, authorizeAddErr
			}
			if !*bool {
				return nil,model.NewAppError("CopyFileInfosToForwardPost", "model.file.copy_file_infos_to_forward_post_authorize.app_error", nil, "赋权失败", http.StatusBadRequest)
			}
		}
		if _, err := a.Srv.Store.FileInfo().Save(fileInfo); err != nil {
			return newFileIds, err
		...
		for _,v := range newFileIds{
			//lgx-20190924 将转发的文件复制到对应的转发频道目录下
			pathPrefix := time.Now().Format("20060102") + "/teams/" + "noteam/" + "channels/" + channelId + "/users/" + fileInfo.CreatorId + "/" + v + "/"
			dstFilePath := pathPrefix + fileInfo.Name
			srcFilePath := fileInfo.Path
			if !*a.Config().FileSettings.EnableCdms && fileInfo.Hash == "" {
				err := a.CopyFile(srcFilePath,dstFilePath)
				if err != nil {
					return nil,err
				}
			}
			// lgx-20190926 如果是图片，在拼接两个path.
			if fileInfo.IsImage() {
				filename := filepath.Base(dstFilePath)
				nameWithoutExtension := filename[:strings.LastIndex(filename, ".")]
				previewPath := pathPrefix + nameWithoutExtension + "_preview.jpg"
				thumbnailPath := pathPrefix + nameWithoutExtension + "_thumb.jpg"

				//lgx-20190926 本地复制预览图片和缩略图的位置
				err := a.CopyFile(fileInfo.PreviewPath,previewPath)
				if err != nil{
					return nil,err
				}
				err = a.CopyFile(fileInfo.ThumbnailPath,thumbnailPath)
				if err != nil{
					return nil,err
				}
				// lgx-20190926 分别是ThumbnailPath和PreviewPath并更新到数据库
				err = a.Srv.Store.FileInfo().UpdateImagePath(v,dstFilePath,channelId,previewPath,thumbnailPath)
			}
			//lgx-20190924 除了图片外，其他文件路径更新到库中
			err = a.Srv.Store.FileInfo().Update(v,dstFilePath,channelId)
			if err != nil{
				return nil,err
			}
		}
...
// lgx-20191010 根据fileIds复制文件，图片。
func (a *App) CopyFileInfosToForward(userId, channelId string, fileIds []string) ([]string, *model.AppError) {
	var newFileIds []string
	now := model.GetMillis()
	for _, fileId := range fileIds {
		fileInfo, err := a.Srv.Store.FileInfo().Get(fileId)
		...
		if *a.Config().FileSettings.EnableCdms && fileInfo.Hash != "" && fileInfo.ChannelId != channelId {
			...
			if !*bool {
				return nil,model.NewAppError("CopyFileInfosToForward", "model.file.copy_file_infos_to_forward_authorize.app_error", nil, "赋权失败", http.StatusBadRequest)
			}
		}
		if _, err := a.Srv.Store.FileInfo().Save(fileInfo); err != nil {
			return newFileIds, err
		...
		//lgx-20190924 将转发的文件复制到对应的转发频道目录下
		pathPrefix := time.Now().Format("20060102") + "/teams/" + "noteam/" + "channels/" + channelId + "/users/" + fileInfo.CreatorId + "/" + fileInfo.Id + "/"
		dstFilePath := pathPrefix + fileInfo.Name
		srcFilePath := fileInfo.Path
		if !*a.Config().FileSettings.EnableCdms && fileInfo.Hash == "" {
			err := a.CopyFile(srcFilePath,dstFilePath)
		...
		if fileInfo.IsImage() {
			...
			// lgx-20190926 分别是ThumbnailPath和PreviewPath并更新到数据库
			err = a.Srv.Store.FileInfo().UpdateImagePath(fileInfo.Id, dstFilePath, channelId, previewPath, thumbnailPath)
		}
		//lgx-20190924 除了图片外，其他文件路径更新到库中
		err = a.Srv.Store.FileInfo().Update(fileInfo.Id, dstFilePath, channelId)
		if err != nil {
			return nil, err
		}
		//}
