...
// Get fetches the given bot in the database.
func (us SqlBotStore) GetByName(botUserName string) (*model.Bot, *model.AppError) {
	var excludeDeletedSql = "AND b.DeleteAt = 0"
	query := `
		SELECT
			b.UserId,
	...
	var bot *model.Bot
	if err := us.GetReplica().SelectOne(&bot, query, map[string]interface{}{"Description": botUserName}); err == sql.ErrNoRows {
		return nil, model.MakeBotNotFoundError(botUserName)
	} else if err != nil {
		return nil, model.NewAppError("SqlBotStore.Get", "store.sql_bot.get.app_error", map[string]interface{}{"Description": botUserName}, err.Error(), http.StatusInternalServerError)
	}
