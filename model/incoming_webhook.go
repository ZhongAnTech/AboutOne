...
type IncomingWebhookRequest struct {
	...
	TeamName        string `json:"teamname"`          //公司id
	TeamDisplayName string `json:"team_display_name"` //公司名称
	//ChannelName string       `json:"channel"`  //项目编码
	ChannelDisplayName string `json:"channel_display_name"` //项目名称
	Purpose            string `json:"purpose"`
	Header             string `json:"header"`
	//Type        string       `json:"type"` //O-公共  P-私有
...
func (o *IncomingWebhookRequest) CreateChannelReqIsValid() *AppError {
	if len(o.TeamName) == 0 ||  len(o.TeamName) > 64{
		return NewAppError("CreatChannelRequest.IsValid", "model.incoming_hook.teamname.app_error", nil, "", http.StatusBadRequest)
	}

	if len(o.ChannelName) == 0 ||  len(o.ChannelName) > 64{
		return NewAppError("CreatChannelRequest.IsValid", "model.incoming_hook.channel.app_error", nil, "", http.StatusBadRequest)
	}

	if len(o.TeamDisplayName) == 0 ||  len(o.TeamDisplayName) > 64{
		return NewAppError("CreatChannelRequest.IsValid", "model.incoming_hook.display_name.app_error", nil, "", http.StatusBadRequest)
	}

	if len(o.ChannelDisplayName) == 0 ||  len(o.ChannelDisplayName) > 64{
		return NewAppError("CreatChannelRequest.IsValid", "model.incoming_hook.display_name.app_error", nil, "", http.StatusBadRequest)
	}

	if len(o.Type) == 0 ||  len(o.Type) > 1{
		return NewAppError("CreatChannelRequest.IsValid", "model.incoming_hook.type.app_error", nil, "", http.StatusBadRequest)
	}

	if len(o.Owner) == 0{
		return NewAppError("CreatChannelRequest.IsValid", "model.incoming_hook.owner.app_error", nil, "", http.StatusBadRequest)
	}
