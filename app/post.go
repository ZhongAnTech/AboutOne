...
func (a *App) attachFilesToPost(post *model.Post) *model.AppError {
	...
	for _, fileId := range post.FileIds {
		...
		info := &model.FileOperationLog{
			...
			Operator: post.UserId,
			Operation:"1",
			CreateAt:post.CreateAt,
		}
		_,err2 := a.Srv.Store.FileOperation().Save(info)
		if err != nil {
			mlog.Warn("Failed to attach file to post", mlog.String("file_id", fileId), mlog.String("post_id", post.Id), mlog.Err(err))
			continue
		}
		if err2 != nil {
			mlog.Warn("Failed to store file_operation_log to post", mlog.String("file_id", fileId), mlog.String("post_id", post.Id), mlog.Err(err))
			continue
		}
...
func (a *App) UpdatePost(post *model.Post, safeUpdate bool) (*model.Post, *model.AppError) {
	...
	*newPost = *oldPost
	...
	if newPost.Message != post.Message {
		...
		newPost.CardType = post.CardType
...
func (a *App) PatchPost(postId string, patch *model.PostPatch) (*model.Post, *model.AppError) {
    ...
    if post.BatchId != "" {
		// 通过post.UUId 获取posts
		posts, err := a.GetPostsForBatchId(post.BatchId)
		if err != nil {
			return nil, err
		}
		for _, v := range posts {
...
func (a *App)GetPostsForBatchId(batchId string)([]*model.Post,*model.AppError) {
	return a.Srv.Store.Post().GetPostsForBatchId(batchId)
}
...
func (a *App) GetLatestPost(channelId string) (*model.Post, *model.AppError) {
	return a.Srv.Store.Post().GetLatestPost(channelId)
}
...
func (a *App) DeletePost(postId, deleteByID string) (*model.Post, *model.AppError) {
	...
	a.Srv.Go(func() {
		...
		for _ ,info := range infos{
			...
			_, err2 := a.Srv.Store.FileOperation().Save(log)
			if err2 != nil {
...
func (a *App) SearchPostsForChannel(channelIds []string,term string,page int, perPage int) ([]*model.Post, *model.AppError){
	return a.Srv.Store.Post().SearchMessage(channelIds,term,page*perPage, perPage)
}

func (a *App) SearchPostsCount(channelIds []string,term string) (model.SearchChatCountRows, *model.AppError){
	return  a.Srv.Store.Post().SearchMessageCount(channelIds,term)
}
