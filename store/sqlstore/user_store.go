...
func NewSqlUserStore(sqlStore SqlStore, metrics einterfaces.MetricsInterface) store.UserStore {
	...
	for _, db := range sqlStore.GetAllConns() {
		...
		table.ColMap("DepartmentName").SetMaxSize(255)
		table.ColMap("JobGrade").SetMaxSize(20)
		table.ColMap("Gender").SetMaxSize(20)
		...
		table.ColMap("Login").SetMaxSize(2)
	}
...
func (us SqlUserStore) UpdateLogin(userId string,login string) *model.AppError{
	curTime := model.GetMillis()
	if _, err := us.GetMaster().Exec("UPDATE Users SET Login = :Login, UpdateAt = :Time WHERE Id = :UserId", map[string]interface{}{"Login": login, "Time": curTime, "UserId": userId}); err != nil {
		return model.NewAppError("SqlUserStore.UpdateLogin", "store.sql_user.update_login.app_error", nil, "login="+login+",user_id="+userId, http.StatusInternalServerError)
	}
	return nil
}

func (us SqlUserStore) SaveOrUpdate(user *model.User) (*model.User,*model.AppError) {
	oldUser :=&model.User{}
	if err := us.GetReplica().SelectOne(oldUser, "SELECT * FROM Users WHERE Username = :Username", map[string]interface{}{"Username": user.Username}); err == nil {
		oldUser.Ou = user.Ou
		...
		oldUser.JobGrade = user.JobGrade
		oldUser.Gender = user.Gender
		oldUser.UpdateAt = time.Now().UnixNano() / int64(time.Millisecond)
		if _, err := us.GetMaster().Update(oldUser); err != nil {
			return nil,model.NewAppError("SqlUserStore.SaveOrUpdate", "store.user.update.app_error", nil, err.Error(), http.StatusInternalServerError)
		}
		user = oldUser
	} else {
		user.PreSave()
		if err := us.GetMaster().Insert(user); err != nil {
			if !(strings.Contains(err.Error(), "for key 'PRIMARY'") && strings.Contains(err.Error(), "Duplicate entry")) {
				return nil,model.NewAppError("SqlUserStore.SaveOrUpdate", "store.user.save.app_error", nil, err.Error(), http.StatusInternalServerError)
			}
		}
	}
	return user,nil
...
func (us SqlUserStore) GetAllUpdateProfiles(options *model.UserGetOptions) store.StoreChannel {
	limitTime := utils.TimeBeforeHours("-24h")
	return store.Do(func(result *store.StoreResult) {
		query := us.getQueryBuilder().
			Select("u.Username,(CASE WHEN u.Ou IS NOT NULL THEN u.Ou ELSE '' END) as Ou ").
			From("Users u").Where("UpdateAt < ?",limitTime).
			Offset(uint64(options.Page * options.PerPage)).Limit(uint64(options.PerPage))

		queryString, args, err := query.ToSql()
		if err != nil {
			result.Err = model.NewAppError("SqlUserStore.GetAllUpdateProfiles", "store.sql_user.app_error", nil, err.Error(), http.StatusInternalServerError)
			return
		}

		var users []*model.User
		if _, err := us.GetReplica().Select(&users, queryString, args...); err != nil {
			result.Err = model.NewAppError("SqlUserStore.GetAllUpdateProfiles", "store.sql_user.get_profiles.app_error", nil, err.Error(), http.StatusInternalServerError)
			return
		}
...
func (us SqlUserStore) GetEntityByUsername(userName string) (*model.User, *model.AppError) {
	userName = strings.ToLower(userName)

	query := us.usersQuery.Where("Username = ?", userName)

	queryString, args, err := query.ToSql()
	if err != nil {
		return nil, model.NewAppError("SqlUserStore.GetEntityByUsername", "store.sql_user.app_error", nil, err.Error(), http.StatusInternalServerError)
	}

	user := model.User{}
	if err := us.GetReplica().SelectOne(&user, queryString, args...); err != nil {
		return nil, model.NewAppError("SqlUserStore.GetEntityByUsername", store.MISSING_ACCOUNT_ERROR, nil, "Username="+userName+", "+err.Error(), http.StatusInternalServerError)
	}
