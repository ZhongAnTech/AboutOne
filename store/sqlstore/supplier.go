...
type SqlSupplierOldStores struct {
	...
	collect              store.CollectStore
	license              store.LicenseStore
	token                store.TokenStore
	emoji                store.EmojiStore
	status               store.StatusStore
	fileInfo             store.FileInfoStore
	fileOperationLog     store.FileOperationLogStore
...
type SqlSupplier struct {
	...
	// See https://za-white-screen/pull/7281
	rrCounter      int64
	srCounter      int64
...
func NewSqlSupplier(settings model.SqlSettings, metrics einterfaces.MetricsInterface) *SqlSupplier {
	...
	supplier.oldStores.meeting = NewSqlMeetingStore(supplier, metrics)
	...
	supplier.oldStores.collect = NewSqlCollectStore(supplier,metrics)
	...
	supplier.oldStores.fileOperationLog = NewSqlFileOperationLogStore(supplier, metrics)
	...
	supplier.oldStores.collect.(*SqlCollectStore).CreateIndexesIfNotExists()
...
func (ss *SqlSupplier) createIndexIfNotExists(indexName string, tableName string, columnNames []string, indexType string, unique bool) bool {
	...
	if ss.DriverName() == model.DATABASE_DRIVER_POSTGRES {
		...
		if err != nil {
			mlog.Critical(fmt.Sprintf("Failed to create index %v,%s", err,tableName))
			time.Sleep(time.Second)
...
func (ss *SqlSupplier) Meeting() store.MeetingStore {
	return ss.oldStores.meeting
}
...
func (ss *SqlSupplier) FileOperation() store.FileOperationLogStore {
	return ss.oldStores.fileOperationLog
}
...
func (ss *SqlSupplier) Collect() store.CollectStore {
	return ss.oldStores.collect
}
