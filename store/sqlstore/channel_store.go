...
func (s SqlChannelStore) DisbandChannelMembers(channelId string) *model.AppError {
	_, err := s.GetMaster().Exec("DELETE FROM ChannelMembers WHERE ChannelId = :ChannelId", map[string]interface{}{"ChannelId": channelId})
	if err != nil {
		return model.NewAppError("SqlChannelStore.DisbandChannelMembers", "store.sql_channel.disband_channel_member.app_error", nil, "channel_id="+channelId+", "+err.Error(), http.StatusInternalServerError)
	}
	return nil
}
...
func (s SqlChannelStore) GetAllChannelsForPosts(offset, limit int) (*model.ChannelListWithPostData, *model.AppError) {
	data := &model.ChannelListWithPostData{}
	_, err := s.GetReplica().Select(data, `
		SELECT
			c.*,
			p.Message,
			p.CreateAt,p.ChannelId
		FROM
			Channels AS c
		LEFT JOIN
			Posts p ON (p.ChannelID = c.Id)
		GROUP BY 
			p.ChannelId
		ORDER BY
			p.CreateAt DESC
	...
	if err != nil {
		return nil, model.NewAppError("SqlChannelStore.GetAllChannelsForPosts", "store.sql_channel.get_all_channels.get.app_error", nil, err.Error(), http.StatusInternalServerError)
	}
	return data, nil
}
...
func (s SqlChannelStore) GetOnlyByName(name string) (*model.Channel, *model.AppError) {
	return s.GetOnlyByChannelName(name)
}
...
func (s SqlChannelStore) GetOnlyByChannelName(name string) (*model.Channel, *model.AppError) {
	channel := model.Channel{}

	if err := s.GetReplica().SelectOne(&channel, "SELECT * FROM Channels WHERE Name = :Name AND DeleteAt = 0", map[string]interface{}{"Name": name}); err != nil {
		if err == sql.ErrNoRows {
			return nil, model.NewAppError("SqlChannelStore.GetDeletedByName", "store.sql_channel.get_deleted_by_name.existing.app_error", nil, "teamId="+""+", "+"name="+name+", "+err.Error(), http.StatusInternalServerError)
		}
		return nil, model.NewAppError("SqlChannelStore.GetDeletedByName", "store.sql_channel.get_deleted_by_name.existing.app_error", nil, "teamId="+""+", "+"name="+name+", "+err.Error(), http.StatusInternalServerError)
	}
...
func (s SqlChannelStore) GetChannelMemberMentions(channelId ,userId string) (*model.ChannelMember, *model.AppError) {
	var channelMember *model.ChannelMember
	_, err := s.GetReplica().Select(&channelMember, `
		SELECT
			PostId
		FROM
			ChannelMembers
...
func (s SqlChannelStore) IncrementMentionCount(postId,channelId string, userId string) *model.AppError {
	_, err := s.GetMaster().Exec(
		...
		map[string]interface{}{"ChannelId": channelId, "UserId": userId,"PostId": postId, "LastUpdateAt": model.GetMillis()})
	if err != nil {
