...
func NewSqlCommandStore(sqlStore SqlStore) store.CommandStore {
	...
	for _, db := range sqlStore.GetAllConns() {
		...
		tableo.ColMap("CardType").SetMaxSize(10)
		tableo.ColMap("CommandType").SetMaxSize(20)
	}
...
func (s SqlCommandStore) GetByTeam(teamId string) ([]*model.Command, *model.AppError) {
	...
	if _, err := s.GetReplica().Select(&commands, "SELECT * FROM Commands WHERE TeamId = :TeamId AND DeleteAt = 0 AND AutoCompleteHint NOT LIKE '%json%'", map[string]interface{}{"TeamId": teamId}); err != nil {
...
func (s SqlCommandStore) GetCommandTypes(TeamId string) ([]*string, *model.AppError) {
	var types []*string

	if _, err := s.GetReplica().Select(&types, "SELECT DISTINCT CommandType FROM Commands WHERE TeamId = :TeamId AND DeleteAt = 0 AND AutoCompleteHint NOT LIKE '%json%'", map[string]interface{}{"TeamId": TeamId}); err != nil {
		return nil, model.NewAppError("SqlCommandStore.GetByTeam", "store.sql_command.save.get_team.app_error", nil, "teamId="+TeamId+", err="+err.Error(), http.StatusInternalServerError)
	}
...
func (s SqlCommandStore) GetAllByTeam(teamId string) ([]*model.Command, *model.AppError) {
	var commands []*model.Command
...
func (s SqlCommandStore) GetAllByTeamAndType(teamId,commandType string) ([]*model.Command, *model.AppError) {
	var commands []*model.Command

	if _, err := s.GetReplica().Select(&commands, "SELECT * FROM Commands WHERE TeamId = :TeamId AND CommandType = :commandType AND DeleteAt = 0 ", map[string]interface{}{"TeamId": teamId,"commandType": commandType}); err != nil {
		return nil, model.NewAppError("SqlCommandStore.GetByTeam", "store.sql_command.save.get_team.app_error", nil, "teamId="+teamId+", err="+err.Error(), http.StatusInternalServerError)
	}
