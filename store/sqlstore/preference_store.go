...
func (s SqlPreferenceStore) GetByCategory(category string,limt int) (model.Preferences, *model.AppError) {
	var preferences model.Preferences
	...
	if _, err := s.GetReplica().Select(&preferences,
		`SELECT
			...
			WHERE
				Category = :Category
				LIMIT :Limit`, map[string]interface{}{"Category": category,"Limit": limt}); err != nil {
		return nil, model.NewAppError("SqlPreferenceStore.GetByCategory", "store.sql_preference.get_by_category.app_error", nil, err.Error(), http.StatusInternalServerError)
	}
