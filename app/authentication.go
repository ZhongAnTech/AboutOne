...
func (a *App) checkLdapUserPasswordAndAllCriteria(userId string, password string, mfaToken string) (*model.User, *model.AppError) {
	if a.Ldap == nil || userId == "" {
		err := model.NewAppError("doLdapAuthentication", "api.user.login_ldap.not_available.app_error", nil, "", http.StatusNotImplemented)
		return nil, err
	}

	ldapUser, err := a.Ldap.DoLogin(userId, password)
	if err != nil {
...
func (a *App) authenticateUser(user *model.User, password, mfaToken string) (*model.User, *model.AppError) {
	...
	if user.AuthService == model.USER_AUTH_SERVICE_LDAP {
		...
		ldapUser, err := a.checkLdapUserPasswordAndAllCriteria(user.Username, password, mfaToken)
		if err != nil {
			err.StatusCode = http.StatusUnauthorized
			return user, err
		}
		if user.Id != ""{
			...
			// save to db
			a.Srv.Store.User().Save(ldapUser)
			return ldapUser,nil
		}
	}
