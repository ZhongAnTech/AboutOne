...
func (a *App) CreateWebhookPostAddCardType(cardType string,userId string, channel *model.Channel, text, overrideUsername, overrideIconUrl string, props model.StringInterface, postType string, postRootId string) (*model.Post, *model.AppError) {
	// parse links into Markdown format
	linkWithTextRegex := regexp.MustCompile(`<([^\n<\|>]+)\|([^\n>]+)>`)
	text = linkWithTextRegex.ReplaceAllString(text, "[${2}](${1})")

	post := &model.Post{ CardType: cardType,UserId: userId, ChannelId: channel.Id, Message: text, Type: postType, RootId: postRootId}
...
func (a *App) HandleIncomingWebhook(hookId string, req *model.IncomingWebhookRequest) *model.AppError {
	...
	_,err := a.CreateWebhookPostAddCardType(req.CardType,hook.UserId, channel, text, overrideUsername, overrideIconUrl, req.Props, webhookType, "")
