...
func NewSqlPostStore(sqlStore SqlStore, metrics einterfaces.MetricsInterface) store.PostStore {
	...
	for _, db := range sqlStore.GetAllConns() {
		...
		table.ColMap("BatchId").SetMaxSize(26)
		...
		table.ColMap("ParentMessage").SetMaxSize(1024)
...
func (s *SqlPostStore) CreateIndexesIfNotExists() {
	...
	s.CreateIndexIfNotExists("idx_posts_batchId", "Posts", "BatchId")
...
func (s *SqlPostStore) Update(newPost *model.Post, oldPost *model.Post) (*model.Post, *model.AppError) {
	...
	oldPost.CardType = newPost.CardType
...
func (s *SqlPostStore) GetSingleContainDeleted(id string) (*model.Post, *model.AppError) {
	var post model.Post
	err := s.GetReplica().SelectOne(&post, "SELECT * FROM Posts WHERE Id = :Id", map[string]interface{}{"Id": id})
	if err != nil {
		return nil, model.NewAppError("SqlPostStore.GetSingleContainDeleted", "store.sql_post.get_single_contain_deleted.app_error", nil, "id="+id+err.Error(), http.StatusNotFound)
	}
	return &post, nil
}
...
func (s *SqlPostStore) GetLatestPost(channelId string) (*model.Post, *model.AppError) {
	var et *model.Post
	err := s.GetReplica().SelectOne(&et, "SELECT * FROM Posts WHERE ChannelId = :ChannelId ORDER BY UpdateAt DESC LIMIT 1", map[string]interface{}{"ChannelId": channelId})
	if err != nil {
		//mlog.Error("GetLatestPost  failed! err="+err.Error())
		return nil,nil
	}
	return et,nil
}
...
func (s *SqlPostStore)GetPostsForBatchId(batchId string) ([]*model.Post, *model.AppError) {
	var posts []*model.Post
	_, err := s.GetReplica().Select(&posts,
		...
		FROM
			Posts
		WHERE
			BatchId = :BatchId AND DeleteAt = 0)
		`,
		map[string]interface{}{"BatchId": batchId})

	if err != nil {
		return nil, model.NewAppError("SqlPostStore.GetPosts", "store.sql_post.get_posts.app_error", nil, "uuid="+batchId+err.Error(), http.StatusInternalServerError)
	}
	return posts, nil
}
	...
	return store.Do(func(result *store.StoreResult) {
		...
		searchQuery := `
			SELECT
				Posts.*,
               (CASE 
							WHEN c.LastViewedAt < Posts.CreateAt THEN '0'
							ELSE '1'
						END
					) as IsRead
			FROM
				Posts
  			LEFT JOIN channelmembers c
        			on c.ChannelId = Posts.ChannelId
                  and c.UserId = :UserId
			WHERE
				Posts.DeleteAt = 0
				AND Posts.Type NOT LIKE '` + model.POST_SYSTEM_MESSAGE_PREFIX + `%'
				POST_FILTER
		...
		if len(params.AfterDate) > 1 || len(params.BeforeDate) > 1 || len(params.OnDate) > 1 {
			if len(params.OnDate) > 1 {
				...
				searchQuery = strings.Replace(searchQuery, "CREATEDATE_CLAUSE", "AND Posts.CreateAt BETWEEN :OnDateStart AND :OnDateEnd ", 1)
				...
				searchQuery = strings.Replace(searchQuery, "CREATEDATE_CLAUSE", "AND Posts.CreateAt >= :AfterDate ", 1)
			} else if len(params.BeforeDate) > 1 {
				beforeDate := params.GetBeforeDateMillis()
				queryParams["BeforeDate"] = strconv.FormatInt(beforeDate, 10)

				// less than `before date`
				searchQuery = strings.Replace(searchQuery, "CREATEDATE_CLAUSE", "AND Posts.CreateAt <= :BeforeDate ", 1)
			}
		} else {
...
//cml-20190903 条件查询分channel统计消息记录数
func (fs SqlPostStore) SearchMessageCount(channelIds []string,term string) (model.SearchChatCountRows, *model.AppError) {
	...
	searchQuery := `SELECT
				...
				Count(Posts.Id) Count
			FROM
				Posts
			WHERE
				Posts.DeleteAt = 0
				CHANNEL_FILTER
				AND Posts.Message like :Term
			GROUP BY
				Posts.ChannelId`
	...
	if len(channelIds) > 1 {
		...
		searchQuery = strings.Replace(searchQuery, "CHANNEL_FILTER", "AND Posts.ChannelId IN ("+inClause+")", 1)
	}else if len(channelIds) == 1  {
		queryParams["InChannel"] = channelIds[0]
		searchQuery = strings.Replace(searchQuery, "CHANNEL_FILTER", "AND Posts.ChannelId = :InChannel", 1)
	}else{
		searchQuery = strings.Replace(searchQuery, "CHANNEL_FILTER", "AND 1=1", 1)
	}

	_,err := fs.GetSearchReplica().Select(&rows,searchQuery,queryParams)
	if err != nil {
		return nil, model.NewAppError("SqlFileInfoStore.GetForPost",
			"store.post_store.search_message_count.app_error", nil, "searchQuery="+searchQuery+", "+err.Error(), http.StatusInternalServerError)
	}
	return rows, nil
}

//cml-20190903 条件查询分页查询
func (fs SqlPostStore) SearchMessage(channelIds []string,term string,offset int, limit int) ([]*model.Post, *model.AppError) {
	var post []*model.Post
	...
	_,err := fs.GetSearchReplica().Select(&post,searchQuery,queryParams)
	if err != nil {
		return nil, model.NewAppError("SqlFileInfoStore.GetForPost",
			"store.post_store.search_message.app_error", nil, "searchQuery="+searchQuery+", "+err.Error(), http.StatusInternalServerError)
	}
	return post, nil
}
...
func (s *SqlPostStore) GetPostsByIdsAsc(postIds []string) ([]*model.Post, *model.AppError) {
	keys, params := MapStringsToQueryParams(postIds, "Post")

	query := `SELECT * FROM Posts WHERE Id IN ` + keys + ` ORDER BY CreateAt ASC`
	...
	if err != nil {
		mlog.Error(fmt.Sprint(err))
		return nil, model.NewAppError("SqlPostStore.GetPostsByIdsAsc", "store.sql_post.get_posts_by_ids.app_error", nil, "", http.StatusInternalServerError)
	}
	return posts, nil
}
