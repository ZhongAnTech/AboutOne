...
func (api *API) InitChannel() {
	...
	api.BaseRoutes.Channels.Handle("/post/all", api.ApiSessionRequired(getAllChannelsForPosts)).Methods("GET")
	...
	api.BaseRoutes.User.Handle("/teams/{team_id:[A-Za-z0-9]+}/channels", api.ApiSessionRequired(getChannelsForTeamForForTerm)).Methods("POST")
	...
	api.BaseRoutes.ChannelMembers.Handle("", api.ApiSessionRequired(addChannelMemberOu)).Methods("POST")
	api.BaseRoutes.ChannelMembers.Handle("", api.ApiSessionRequired(disbandChannel)).Methods("DELETE")
...
func disbandChannel(c *Context,w http.ResponseWriter,r *http.Request)  {
	c.RequireChannelId()
	if c.Err != nil{
		return
	}
	...
	err = c.App.DisbandChannelMembers(channel, c.App.Session.UserId)
	if err != nil {
		c.Err = err
		return
	}
...
func getAllChannelsForPosts(c *Context, w http.ResponseWriter, r *http.Request) {
	if !c.App.SessionHasPermissionTo(c.App.Session, model.PERMISSION_MANAGE_SYSTEM) {
		c.SetPermissionError(model.PERMISSION_MANAGE_SYSTEM)
		return
	}

	channels, err := c.App.GetAllChannelsForPosts(c.Params.Page, c.Params.PerPage)
	if err != nil {
		c.Err = err
		return
	}
...
func getChannelsForTeamForForTerm(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	//循环遍历删除不包含term的channle
	result,_ :=json.Marshal(channels.FilteByTerm(props.Term))
...
func getChannelsForTeamForUser(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	for _, channel := range *channels {
		//channelMentionsProp := make(map[string]interface{}, 1)
		mentions, _ := c.App.GetChannelMemberMentions(channel.Id, c.Params.UserId)
		mention := ""
		post := &model.Post{}
		if mentions != nil && mentions.PostId != "" {
			post, _ = c.App.GetSinglePost(mentions.PostId)
		} else {
			// get last posts
			post, _ = c.App.GetLatestPost(channel.Id)
		}
		if post != nil {
			filedLen := len(post.FileIds)
			if filedLen > 0 {
				fileId := post.FileIds[filedLen-1]
				fileInfo, _ := c.App.GetFileInfo(fileId)
				if fileInfo != nil {
					file := model.StringArray{fileInfo.Extension}
					post.FileIds = file
				}
			}
			byte, _ := json.Marshal(post)
			mention = string(byte)
			me := &model.Channel{}
			me.DisplayName = mention
			channel.AddProp("resume", me.DisplayName)
		}
	}
	if c.HandleEtag(channels.Etag(), "Get Channels", w, r) {
		return
...
func addChannelMemberOu(c *Context, w http.ResponseWriter, r *http.Request) {
	c.RequireChannelId()
	if c.Err != nil {
		return
	}
	...
	props := model.StringInterfaceFromJson(r.Body)
	userIdProps, ok := props["user_id"].(string)

	if !ok {
		c.SetInvalidParam("user_id(props)")
	}
	...
	if len(userIdProps) != 26 {
		ujson := []byte(userIdProps)
		var uob userObject
		json.Unmarshal(ujson, &uob)
		if uob.UserId == "" {
			c.SetInvalidParam("user_id")
			return
		}
		...
		// check user is exits?
		if result := <-c.App.Srv.Store.User().GetByUsername(user.Username); result.Err != nil {
			// get user info from ua
			c.App.SSO.GetUserCompanyInfo(user)
			result := <-c.App.Srv.Store.User().Save(user)
			if result.Err != nil {
				fmt.Println("Couldn't save the user err=%v", result.Err)
				return
			}
		} else {
			user = result.Data.(*model.User)
		}
		// check user is belong to this team
		if channel.TeamId != "" {
			team, _ := c.App.Srv.Store.Team().Get(channel.TeamId)
			if team == nil {
				mlog.Error("channel team not found!")
				return
			}
			if team.Name == user.Ou {
				c.App.AddTeamMember(channel.TeamId, user.Id)
			}
		}
		userId = user.Id
	}else{
