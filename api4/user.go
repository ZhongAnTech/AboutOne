...
const (
	MAXIMUM_UPLOAD_SOFTPACKAGE_FILE_SIZE = 50 * 1024 * 1024
)
...
func getUser(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	c.App.SSO.GetUserDepartmentInfo(user)
	if err != nil {
		c.Err = err
		return
	}
...
func login(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if teams == nil || len(teams) == 0{
		// get teams
		teams,err := c.App.GetTeamsByName(user.Ou)
		if err!=nil{
			return
		}
		if teams == nil || len(teams) == 0{
		...
		// and user to team
		c.App.AddTeamMember(teams[0].Id,user.Id)
	}
